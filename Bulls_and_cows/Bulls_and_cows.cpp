...
сив(М.) в функцию передавать не надо. Пусть размер массива тоже будет глоб.к., чтобы и его не надо было передавать в функции.
(!) когда узнаем как можно создавать массивы переменной длины, можно будет выбирать длину числа.
Пользовательское число необходимо нарезать, а загаданное число будет создаваться в массиве. Создам глоб.к. через define size. Создам глоб.М., хранящий загаданное число
(глоб.объекты по умолчанию пустые). */
int number[size];
int value[size];					// глоб.М., хранящий введенное игроком число

void SetNumber();
/* Функция загадывания числа. Т.к. массив изменяется в функции, число будет сразу в массив помещаться, в каждую ячейку каждая циферка. Не нужно ничего возвращать. Пере-
давать в функцию ничего не нужно т.к. массив number глобальный, а size это define (там где будет size, само подставится четверкой) */

void Slice(int);
/* Функция преобразования (нарезки) числа. Она будет преобразовывать число, введенное пользователем в массив цифр. value - глоб.М.
